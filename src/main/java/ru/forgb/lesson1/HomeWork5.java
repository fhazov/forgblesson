...
class AppData {
    ...
    /*public void saveCSVOld (String fileName) {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(fileName))) {
            StringBuilder sb = new StringBuilder();
            String columnNamesList = "Id,Name";
            sb.append(columnNamesList +"\n");
            sb.append(rowToStringCSV(header));
            writer.append(sb.toString());

            for (Integer[] row : data) {
                writer.append(rowToStringCSV(row));
            }
    ...
    public void saveCSV(String fileName) {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(fileName))) {
            writer.write(rowToStringCSV(header));

            for (Integer[] row : data) {
                writer.write(rowToStringCSV(row));
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    private <T> String rowToStringCSV(T[] row) {
        String result = "";
